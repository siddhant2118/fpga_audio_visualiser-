...
// MIC3 SPI capture, DC removal, noise gate, moving average filter
// LED bar display and 7-segment output
//

module audio_input_pipeline(
    input  wire        clk,          
    ...
    wire cs;
    mic_sample_clock u_cs(.CLK(clk), .cs(cs));
    assign J_MIC3_Pin1 = cs;

    wire [11:0] mic_in;
    wire        sclk;
    mic_spi_capture u_cap(
        .CLK(clk),
        .cs(cs),
        .MISO(J_MIC3_Pin3),
        .clk_samp(),         
    ...
    wire [15:0] maxvalue;
    audio_peak_detector u_peak(.CLK(clk), .mic_in(mic_in), .maxvalue(maxvalue));
    ...
    sample_frame_packer #(.N_SAMPLES(256)) u_pack (
        .clk(clk),
    ...
    sample_buffer_ram #(.N_SAMPLES(256)) u_bram (
        .clk(clk),
        .wr_en(wr_en_w),
        .wr_addr(wr_addr_w),
        .wr_data(wr_data_w),
